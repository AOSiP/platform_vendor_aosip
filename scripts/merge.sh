...
BRANCH=android-11.0.0_r${1}

# Manifest branch
AOSIP_MANIFEST=aosip.xml
...
blacklist=('external/nano manifest prebuilts/clang/host/linux-x86 prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9 prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9 prebuilts/gcc/linux-x86/host/x86_64-linux-glibc2.17-4.8 prebuilts/tools prebuilts/tools-lineage vendor/aosip vendor/overlay vendor/gapps')
...
function get_repos() {
  ...
  do
    if grep -q "$i" /tmp/rebase.tmp; then # If Google has it and
      if grep -q "$i" $WORKING_DIR/manifest/snippets/$AOSIP_MANIFEST; then # If we have it in our manifest and
        if grep "$i" $WORKING_DIR/manifest/snippets/$AOSIP_MANIFEST | grep -q "remote="; then # If we track our own copy of it
